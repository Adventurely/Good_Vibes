/* One Durable Object for the whole of Sunward's board.
 *
 ...
 * one answer rather than a merge of several. The Worker reaches it by name —
 * `idFromName('sunward')` — so every request anywhere in the world lands on
 * the same rows.
 *
 * It is thin on purpose. Every decision — what a name may be, what a figure
 * may be, who is too soon, who is pruned — is in `src/sunward-board.js`, the
 * same module the dev server calls, so the two cannot disagree. This file
 * reads the request, hands it over, saves if anything changed, and writes the
 * answer.
 */
...
export class SunwardBoard extends DurableObject {
  constructor(ctx, env){
    super(ctx, env);
    /* One key, the whole map. MAX_PLAYERS in the rules module is what keeps
       it under the size a value may be; if that cap ever moves up by an order
       of magnitude, this becomes a key per row and a list() on load. */
    this.store = { players: {} };

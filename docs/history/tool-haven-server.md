> **Historical.** This describes the Cloudflare Worker that used to serve this
> game as a tool inside `Adventurely/Tool-Haven`, back when this repository was
> a single game copied into a tool manager. That pipeline is gone: the repo is a
> stand-alone server now (see the [README](../../README.md)), and nothing here
> is deployed from it or copied anywhere.
>
> It is kept because the design it records is still worth reading — what a
> persisting, authenticated host has to add on top of the in-memory room in
> `games/good-vibes/server/`, and which pieces of room state cannot be rebuilt
> from the room's name. Paths in it refer to the old layout: `src/rooms.js` is
> now `games/good-vibes/server/`, and `public/content.js` is now
> `games/good-vibes/public/shared/`.

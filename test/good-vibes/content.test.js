...
} from '../../games/good-vibes/public/art.js';
import {
...
/* The shared rules are imported by the authoritative room object, not
...
/* This barrel is imported by the room and by the browser, and an import of a name
...
test('every name this module has ever published is still exported', async () => {
  const content = await import('../../games/good-vibes/public/shared/index.js');
  const missing = PUBLISHED.filter((name) => content[name] === undefined);
  assert.deepEqual(missing, [],
    `dropped from the published contract: ${missing.join(', ')}. `
    + 'Both the room and the browser import this barrel, and a missing export is '
    + 'a throw at the top of whichever one asked for it. Shim it instead.');
});
